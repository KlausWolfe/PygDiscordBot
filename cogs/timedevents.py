...
class TimedEventsCog(commands.Cog, name="timed_events"):
            ...
            if (user is not None) and not (user == self.bot.user) and (user.id not in self.blacklist_list):
                ...
                if (dm_channel is not None):
                    ...
                    if (last_message is not None) and (last_message.author != self.bot.user):
                        ...
                        last_message_text = f"{last_message.author.name}: {last_message.content}\n"
                        if os.path.exists(path):
                            with open(path, 'a+', encoding="utf-8") as f:
                                f.seek(0)
                                lines = f.readlines()[-1:]
                                if (last_message_text not in lines[0]):
                                    f.write(last_message_text)
                            response = await self.bot.get_cog("chatbot").chat_command(last_message, last_message.content)
                            print(f"{user.name} was sent: {response} by {self.char_name}")
    ...
    async def timed_channel(self):
        channels = self.channel_id.split(',')
